...
export const getSurveys = createAsyncThunk("getSurveys", async (state) => {
	const response = nodeAPI.get("/survey/getByUserId/" + jwtDecode(localStorage.getItem("token")).Id);
	return response;
});
...
export const getSurveyCriterias = createAsyncThunk("getSurveyCriterias", async (state) => {
	const response = await nodeAPI.get("/criteria/getCriterias/" + state);
	return response;
});

export const getSurveyItemCriteria = createAsyncThunk("getSurveyItemCriterias", async (state) => {
	const response = await nodeAPI.get("/itemcriteria/getItemCriteria/" + state);
	return response;
});
...
export const getSurveyQuestions = createAsyncThunk("getSurveyQuestions", async (state) => {
...
export const getSurveyResultsByDemografik = createAsyncThunk("getSurveyResultsByDemografik", async (state) => {
	const response = await nodeAPI.get("/surveyResults/getSurveyResultsByDemografik/" + state);
	return response;
});
...
export const releaseSurvey = createAsyncThunk("releaseSurvey", async (state) => {
		...
		builder.addCase(releaseSurvey.fulfilled, (state, action) => {
			if (action.payload.data.success) {
				Swal.fire({
					...
					action.payload.history.go(getLayoutName(action.payload.history) + "/surveys");
				});
			}
		});
	},
